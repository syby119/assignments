 ...
 */
GLuint Shader::_createShader(const std::string& code, GLenum shaderType) {
 ...
 */
void Shader::_createShaderProgram(const std::string& vsCode, const std::string& fsCode) {
    GLuint vs = 0, fs = 0;
    try {
        vs = _createShader(vsCode, GL_VERTEX_SHADER);
        if (!vs) {
